...
mod tests {
    ...
    #[test]
    fn normalize_session_v0917_<redacted-session-id>_aggregates_ai_title_events() {
        // v0.9.17: 端到端 fixture-driven 测试 — 验证 normalize_session batch 入口
        // 对真实 <redacted-session-id> slice (2000 lines, head -2000) 的聚合行为。
        //
        // fixture 包含 (实测 head -2000):
        //   - 108 ai-title events (无 custom-title)
        //   - 3 unique titles: <redacted-project> (65) / <redacted-slug> (35)
        //     / <redacted> <redacted-title> (8)
        //   - 2 title_changes (3 unique - 1 first)
        //   - 末次 title: <redacted-slug> (line 1999 附近)
        //   - first_seen: <redacted> <redacted-title> (line 8)
        use std::path::PathBuf;
        let fixture_path = PathBuf::from("../<redacted-fixture>-v0917.jsonl");
        ...
        // 1) 一次性读所有 records (跟 transcript.rs batch path 同 pattern)
        let mut records: Vec<serde_json::Value> = Vec::new();
        crate::parser::jsonl::for_each_line(&fixture_path, |_idx, _byte, v| {
            records.push(v.clone());
        })
        .expect("read fixture");

        // 2) 跑 normalize_session batch
        let messages = normalize_session(records);

        // 3) 末尾 emit 1 个 ai-title.chart meta (其它 user/assistant/tool 等都正常 emit)
        let chart = messages
            .iter()
            .find(|m| {
                m.blocks[0].data.get("label").and_then(|v| v.as_str()) == Some("ai-title.chart")
            })
            .expect("ai-title.chart meta should exist");
        ...
        // 4) 顶层 stats 验证
        assert_eq!(data.get("event_count").unwrap().as_u64().unwrap(), 108);
        assert_eq!(data.get("unique_title_count").unwrap().as_u64().unwrap(), 3);
        assert_eq!(data.get("custom_title_count").unwrap().as_u64().unwrap(), 0);
        assert_eq!(data.get("ai_title_count").unwrap().as_u64().unwrap(), 108);
        assert_eq!(
            data.get("title_changes_count").unwrap().as_u64().unwrap(),
            2
        );

        // current_title 末次 (highest-precedence 末次, 只有 ai-title, 取最后一条)
        assert_eq!(
            data.get("current_title").unwrap().as_str().unwrap(),
            "<redacted-slug>"
        );
        // first_seen_title 第 1 条 (line 8 附近, Chinese title)
        assert_eq!(
            data.get("first_seen_title").unwrap().as_str().unwrap(),
            "<redacted> <redacted-title>"
        );

        // buckets: clamp(1, 60) = 60 但 108 records → base=1, extra=48
        // → 60 buckets (因为 base=1+1=2 的有 48 个, base=1 的有 12 个, 共 60 个)
        // 实际: base_size = 108/60 = 1, extra_count = 108 % 60 = 48
        // → 60 buckets, 前 48 个 size=2, 后 12 个 size=1
        let buckets = data.get("buckets").unwrap().as_array().unwrap();
        assert_eq!(buckets.len(), 60);
        let total_in_buckets: u64 = buckets
            .iter()
            .map(|b| b.get("event_count").unwrap().as_u64().unwrap())
            .sum();
        assert_eq!(total_in_buckets, 108);

        // top_titles: top 10 by event count
        let top = data.get("top_titles").unwrap().as_array().unwrap();
        assert_eq!(top.len(), 3); // 只有 3 unique
        assert_eq!(
            top[0].get("title").unwrap().as_str().unwrap(),
            "<redacted-project>"
        );
        assert_eq!(top[0].get("event_count").unwrap().as_u64().unwrap(), 65);
        assert_eq!(
            top[1].get("title").unwrap().as_str().unwrap(),
            "<redacted-slug>"
        );
        assert_eq!(top[1].get("event_count").unwrap().as_u64().unwrap(), 35);

        // title_timeline: 按 first_seen index 升序
        let timeline = data.get("title_timeline").unwrap().as_array().unwrap();
        assert_eq!(timeline.len(), 3);
        // 第 1 个 first_seen = <redacted>... (line 8, idx~7)
        assert_eq!(
            timeline[0].get("title").unwrap().as_str().unwrap(),
            "<redacted> <redacted-title>"
        );
        // 第 3 个 first_seen = <redacted-slug> (line 1401, idx~1400)
        assert_eq!(
            timeline[2].get("title").unwrap().as_str().unwrap(),
            "<redacted-slug>"
        );

        // payload.raw_events: 108 records > 10 → head 5 + tail 5
        let payload = data.get("payload").unwrap().as_object().unwrap();
        let raw = payload.get("raw_events").unwrap().as_array().unwrap();
        assert_eq!(raw.len(), 10);
        assert_eq!(payload.get("raw_count").unwrap().as_u64().unwrap(), 108);
    }
}
