/**
 * v0.9.20 (M3): AttachmentBlock — L4 layer
 *
 * 13 个 Claude attachment kind 走统一 slate accent 的 `<AttachmentBlock>`:
 * - agent_listing / agent_listing_delta
 * - skill_listing
 * - plan_mode (含 reveal 入口 + 失败提示)
 * - file_snapshot / file-history-snapshot (默认 5 行 + 折叠)
 * - pr_link / pr-link
 * - agent_name / agent-name
 * - task_reminder (id / description / activeForm / blocks / blockedBy)
 * - invoked_skills
 * - plan_file_reference / compact_file_reference / attached_file
 * - queued_command / queue_operation
 *
 * 13 kind 全部走统一 `.attachment-block-meta` wrapper(M1 决定),共享
 * `slate` accent(`META_ACCENT.attachment`)。
 *
 * 共享 helper (从 MetaBlock.tsx 拆过来):
 * - FileSnapshotBlock: file_snapshot kind 用,默认 5 行 + 折叠
 * - FilePathClickable: 任意路径点击 reveal,带可操作错误 UI
 * - PlanFilePath: plan_mode 专属 — filename + reveal button + 失败 UI
 * - RevealErrorActions: reveal 失败时三个按钮 (复制路径 / 去设置 / 一键开启)
 *
 * 路由:`MetaBlock` 顶层 switch 通过 `ATTACHMENT_META_LABELS` Set 判断
 * (来自 `theme/meta-palette.ts`) → 走 `<AttachmentBlock block={block}
 * parentJsonlPath={parentJsonlPath} />`。
 *
 * fallback: 未识别的 attachment kind(理论上不该发生,`ATTACHMENT_META_LABELS`
 * 是完整集合)走 `<UnknownBlockCard>` 兜底。
 */
