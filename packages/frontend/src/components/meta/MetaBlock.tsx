...
/* v0.9.16: todos.chart 专属渲染
 *
 * dcwin11 bpm-large (6040 行) 57 个 tools.update_store event (LLM plan
 * snapshot 全文,key 100% "todo")。v0.9.8 batch path 把每个 event 单独 emit 成
 * raw meta block,详情页被 57 个 todo 元数据撑爆。本版末尾聚合为 1 个 todos.chart
 * meta:**
 * - 顶层 stats: update_count / unique_task_count / current_done/in_progress/pending
 *   (末次 snapshot 状态) / total_done (跨 session 累计) / churn_count /
 *   churn_add_count / churn_remove_count / duration_ms
 * - buckets[]: 60 个时间窗口,每个含 item_count / done_count /
 *   in_progress_count / pending_count (前端 inline SVG stacked bar 渲染)
 * - completed_tasks[]: 每个 unique title → 第一次 seen as "done" 的
 *   时间戳 + update_index (按 done 时间排序)
 * - churn_events[]: add (新 title 第一次出现) / remove (已知 title 消失)
 *   事件,带 time + title + action
 * - payload.raw_events: 前 5 + 后 5 raw event (drill-down)
 *
 * 渲染策略:
 * - 头部: update_count + unique_task_count pill + current_done/in_progress/pending
 *   pill + churn_count + duration_ms
 * - 中部: TodoChartSvg (60 bar stacked: pending 灰底 + in_progress 蓝中 + done 绿顶)
 * - completed_tasks: 默认显示前 8 个 (跟 v0.9.15 drift events 同 catch pattern)
 * - churn_events: add (绿 .meta-tag-add) / remove (红 .meta-tag-remove) 配色
 * - 折叠 / 展开 raw events (payload.raw_events) — 跟 v0.9.14 / v0.9.15 同 pattern
 *
 * fallback: 缺 update_count 或 buckets → UnknownBlockCard
 */
function TodoChartMetaBlock({ block }: { block: NormalizedBlockFE }) {
  const blk = block as Record<string, unknown>;
  const pl = (block.payload ?? {}) as Record<string, unknown>;
  const get = (...keys: string[]): unknown => {
    for (const k of keys) {
  ...
  const updateCount = num(get("update_count", "updateCount"));
  const uniqueTaskCount = num(get("unique_task_count", "uniqueTaskCount")) ?? 0;
  const currentDone = num(get("current_done", "currentDone")) ?? 0;
  const currentInProgress = num(get("current_in_progress", "currentInProgress")) ?? 0;
  const currentPending = num(get("current_pending", "currentPending")) ?? 0;
  const totalDone = num(get("total_done", "totalDone")) ?? 0;
  const churnCount = num(get("churn_count", "churnCount")) ?? 0;
  const churnAddCount = num(get("churn_add_count", "churnAddCount")) ?? 0;
  const churnRemoveCount = num(get("churn_remove_count", "churnRemoveCount")) ?? 0;
  const durationMs = num(get("duration_ms", "durationMs")) ?? 0;
  const buckets = (get("buckets") as Array<Record<string, unknown>>) ?? [];
  const completedTasks =
    (get("completed_tasks", "completedTasks") as Array<Record<string, unknown>>) ?? [];
  const churnEvents = (get("churn_events", "churnEvents") as Array<Record<string, unknown>>) ?? [];
  const rawEvents = (pl.raw_events as Array<Record<string, unknown>>) ?? [];
  const rawCount = (pl.raw_count as number) ?? rawEvents.length;
