...
```bash
cd $REPO/experiment/embed-db/web
...
> G1 PoC 已合并到主项目 `packages/frontend/`(commit `bc24a08` M1 / `683e61d` M2)。**当前权威状态**、状态表、合并后变更、文件映射、验收、双源对照、M3 TODO 等全部统一在 [embed-db-findings.md](./embed-db-findings.md) 与 [CHANGELOG.md](../../CHANGELOG.md) `[Unreleased]`,本 addendum 不重复。
>
> 主要差异速查:路由改 `useSearchParams('?view=...')` / 数据源改 zustand `useGraphStore` / 标题存储改 `useTitleStore` / 详情面板加跳主项目原生 `/session/:id` + subagent `?path=` 模板 / 三个依赖 (`react-force-graph-2d` / `d3` / `recharts`) 装到主项目 / CSS token 化。
